...
void PhysicalLayer::init(const std::pair<State, Parms>& qp)
{
  ...
  else if (p.pLayerType == "FastUnicycle")
    controlFuncPointer = &(fastUnicycleControl);
  else
...
{
  Control c = standardUnicycle::computeControl(sigma, q, p, sList, idx, debug);
...
void fastUnicycleControl(const State& q, State& nextQ, const int& idx, const Parms& p, const Maneuver& sigma, const List<Sensing>& sList, const bool& debug)
{
  Control c = fastUnicycle::computeControl(sigma, q, p, sList, idx, debug);
